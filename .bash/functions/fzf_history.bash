...
hf() {
  ...
  selected_output=$(cat ~/.bash_history | filter_history | awk '!seen[$0]++' | tac | fzf \
    ...
    --bind "ctrl-r:execute-silent(echo -e '\033[1;32m履歴を更新しました\033[0m' >&2)+reload(cat ~/.bash_history | grep -a -vE '^HISTFILE=|^HISTCONTROL=|^shopt -s histappend|^HISTSIZE=|^HISTFILESIZE=|^PROMPT_COMMAND=\"history -a|^SAVEHIST=|^setopt|^set -o history|^#|^HISTIGNORE=' | awk '!seen[\\\$0]++' | tac)" \
    --expect=tab,enter)
