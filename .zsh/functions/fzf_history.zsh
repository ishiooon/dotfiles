...
hf() {
  ...
  selected_output=$(cat ~/.zsh_history | cut -d';' -f2- | awk '!seen[$0]++' | tac | fzf \
    --layout=reverse \
    --border=rounded \
    --prompt="🔍 " \
    --pointer="▶" \
    ...
    --bind "ctrl-r:execute-silent(echo -e '\033[1;32m履歴を更新しました\033[0m' >&2)+reload(cat ~/.zsh_history | cut -d';' -f2- | awk '!seen[\$0]++' | tac)" \
    --expect=tab,enter)
